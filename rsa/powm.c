...
void MontExpCheck(mpz_t rop, const mpz_t base, const mpz_t exp, const mpz_t N, void(*fp) (mpz_t T, const mpz_t x, const mpz_t y, const mpz_t N, const mp_limb_t N_1))
{
	/*************************************************
		description:使用 蒙哥马利算法 计算 rop = base^exp mod N
		inputParam :
	...
	*************************************************/
	...
	fp(P, K, base, N, N_1);	//将base转化为蒙哥马利数 P
	fp(R, K, temp, N, N_1);	//将  1 转化为蒙哥马利数 R
	for (index = 0; index < ebit; index++) {
		if (mpz_tstbit(exp, index) == 1)
			fp(R, R, P, N, N_1);	//R = R*P mod N
		fp(P, P, P, N, N_1);	//P = P*P mod N
	}
	fp(rop, temp, R, N, N_1);	//将R转化回实数域
...
void MontPro(mpz_t T, const mpz_t x, const mpz_t y, const  mpz_t N, const mp_limb_t N_1)	//Montgomery Product t = MontMul(x, y, mod)
{
	/*************************************************
		description:使用 2位精度 REDC 计算 x*y*r^(-1) mod N
					效率远远低于32比特版本
		inputParam :
					x, y	操作数1, 2, 假定为蒙哥马利数
					N		模数
...
void Mont_Test()
{
	...
	t1 = clock();
	for (i = 0; i < TESTCOUNT; i++) {
		MontExpCheck(r1, base, exp, mod, MontPro);
	}
	t2 = clock();
	printf("2  bit : \t%lf\n", (double)(t2 - t1) / TESTCOUNT);
	...
	t1 = clock();
	for (i = 0; i < TESTCOUNT; i++) {
		MontExpCheck(r1, base, exp, mod, MontPro1);
	}
	t2 = clock();
	printf("32 bit : \t%lf\n", (double)(t2 - t1) / TESTCOUNT);
	...
	t1 = clock();
	for (i = 0; i < TESTCOUNT; i++) {
		MontExpCheck(r1, base, exp, mod, MontPro2);
	}
	t2 = clock();
