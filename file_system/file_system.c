...
int file_encrypt(RSAPublicKey *RSApubKey)
{
	...
	******************************************/
    ...
    //生成主密钥和初始向量IV
    {
		file_init_session(sessionKey, &initVect); //生成会话参数
    ...
    //并写入fp2文件头部
    {
		...
		mpz_mul_2exp(sessionInfo, sessionInfo, SPN_KEY_LENGTH); //左移sessionInfo为sessionKey腾出空间
...
int file_system()
{
    ...
    while (op)
    {
		...
		printf("3.使用RSA加密数据 (pkcs#1 padding)\n");
		printf("4.使用公钥加密文件\n");
		printf("5.使用私钥解密文件\n");
		printf("0.返回上级菜单:\n");
		scanf("%d", &op);
